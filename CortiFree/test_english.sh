...
# Script to test CortiFree app in English
# This script launches the app in the iOS Simulator with English language settings

echo "🇬🇧 Testing CortiFree in English..."
...
echo "Setting device language to English..."
xcrun simctl spawn "$DEVICE_ID" defaults write NSGlobalDomain AppleLanguages -array "en"
xcrun simctl spawn "$DEVICE_ID" defaults write NSGlobalDomain AppleLocale -string "en_US"

# Install the app
echo "Installing app..."
xcrun simctl install "$DEVICE_ID" "$APP_BUNDLE"

# Launch the app with English language
echo "Launching CortiFree in English..."
xcrun simctl launch "$DEVICE_ID" Josbiot.App.CortiFree --args -AppleLanguages "(en)" -AppleLocale "en_US"

echo "✅ CortiFree is now running in English!"
echo ""
echo "To switch back to French, run:"
echo "  ./test_french.sh"
echo ""
echo "Or manually in the iOS Simulator:"
echo "  Settings > General > Language & Region > iPhone Language > Français"