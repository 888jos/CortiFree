...
struct JournalView: View {
    ...
    @State private var showPreviousEntries = false
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 0) {
                ...
                // Zone d'écriture (différente pour gratitude)
                if meditationType == "gratitude" {
                    gratitudeSection
                } else {
                    writingSection
                }
    ...
    private var gratitudeSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Ce pour quoi tu es reconnaissant")
                .font(.custom("Poppins-SemiBold", size: 16))
                .foregroundColor(.white)

            // Liste des items de gratitude
            VStack(spacing: 8) {
                ForEach(Array(gratitudeItems.enumerated()), id: \.offset) { index, item in
                    HStack(spacing: 12) {
                        ...
                        Button(action: {
                            gratitudeItems.remove(at: index)
                        }) {
            ...
            // Input pour nouveau item
            HStack(spacing: 12) {
                TextField("Ajoute quelque chose...", text: $newGratitudeItem, onCommit: addGratitudeItem)
                    .font(.custom("Poppins-Regular", size: 15))
                    .foregroundColor(.white)
                    .padding(14)
                    .background(
                        RoundedRectangle(cornerRadius: 12)
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 12)
                                    .stroke(isGratitudeFieldFocused ? Color(hex: "FF6B9D") : Color.white.opacity(0.1), lineWidth: 1)
                            )
                    )
                ...
                Button(action: addGratitudeItem) {
                    Image(systemName: "paperplane.fill")
                        .font(.system(size: 22))
                        .foregroundColor(newGratitudeItem.isEmpty ? Color.white.opacity(0.3) : Color(hex: "FF6B9D"))
                }
                .disabled(newGratitudeItem.isEmpty)
            }
            ...
            HStack {
                HStack(spacing: 4) {
                    Image(systemName: "heart.fill")
                        .font(.system(size: 10))
                        .foregroundColor(Color(hex: "FF6B9D").opacity(0.7))
                    Text("\(gratitudeItems.count) chose\(gratitudeItems.count > 1 ? "s" : "")")
                        .font(.custom("Poppins-Medium", size: 12))
                        .foregroundColor(Color(hex: "FF6B9D").opacity(0.7))
                }
                Spacer()
    ...
    private func addGratitudeItem() {
        guard !newGratitudeItem.trimmingCharacters(in: .whitespaces).isEmpty else { return }
        gratitudeItems.append(newGratitudeItem)
        newGratitudeItem = ""
    ...
    private var canSave: Bool {
        if meditationType == "gratitude" {
            ...
            return !newEntryContent.isEmpty
        }
    }
            ...
            if !viewModel.entries.isEmpty {
                ...
                Button(action: {
                    withAnimation(.spring(response: 0.3)) {
                        showPreviousEntries.toggle()
                    }
                }) {
                    HStack {
                        Image(systemName: showPreviousEntries ? "chevron.down" : "chevron.right")
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 12)
                            .fill(Color(hex: "1A1B3A").opacity(0.4))
                    )
                }
                ...
                // Entrées (affichées seulement si showPreviousEntries est true)
    ...
    private func saveEntry() async {
        // Pour gratitude, on construit le contenu à partir de la liste
        let contentToSave: String
        if meditationType == "gratitude" {
            contentToSave = gratitudeItems.enumerated().map { index, item in
                "\(index + 1). \(item)"
            }.joined(separator: "\n")
        } else {
            contentToSave = newEntryContent
        }
