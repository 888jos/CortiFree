...
struct JournalHomeView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if selectedTab != .all && selectedTab != .todo && !filteredEntries.isEmpty {
                VStack {
                    Spacer()
                    HStack {
                        Spacer()
    ...
    private var statsSection: some View {
        VStack(spacing: 12) {
            HStack(spacing: 12) {
                StatCard(
                    title: selectedTab == .all ? "Total" : selectedTab.rawValue,
    ...
    private var emptyStateView: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                Text(selectedTab == .all ? "Votre journal est vide" : "Aucune entrée pour \(selectedTab.rawValue)")
                    .font(.custom("Poppins-SemiBold", size: 20))
                    .foregroundColor(.white)
            ...
            // Bouton pour créer une entrée si c'est un onglet spécifique
            if selectedTab != .all {
                Button(action: { showNewEntry = true }) {
                    HStack(spacing: 8) {
    ...
    private var entryPrompt: String {
        ...
        case .clarity: return DailyPrompt.getDailyPrompt(for: "clarity")
        case .reflection: return DailyPrompt.getDailyPrompt(for: "reflection")
        case .goals: return DailyPrompt.getDailyPrompt(for: "goals")
        case .general, .all: return DailyPrompt.getDailyPrompt(for: "general")
        }
    }
